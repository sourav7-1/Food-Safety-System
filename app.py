...
def create_app(config_class=Config):
    ...
    from routes.inspector import (
        inspector_bp,
        inspector_complaints_bp,
        inspector_notifications_bp,
    )
    ...
    app.register_blueprint(inspector_notifications_bp)
    ...
    def set_security_headers(response):
        ...
        # (street basemap) and server.arcgisonline.com (Esri World
        # Imagery satellite basemap) for the Leaflet maps on the admin
        # stall-location picker and the customer /nearby page -- those
        # render raster tile images fetched directly by the browser as
        # <img> requests, which img-src 'self' data: alone would
        # otherwise block (the map would render with no basemap under
        # the pins). Nothing else changed: still no img-src wildcard,
        # still no new script-src origin (Leaflet itself loads from the
        # already-allowlisted cdn.jsdelivr.net).
